...
const PATHS = {
	...
	submodulesConfig: path.join(pluginRoot, 'Config', 'SubModulesConfig.ini'),
...
function parseSubModulesConfig(filePath) {
	if (!fs.existsSync(filePath)) {
		console.error(`SubModulesConfig.ini not found at: ${filePath}`);
		process.exit(1);
	}
...
function loadExistingUplugin() {
	...
	return JSON.parse(fs.readFileSync(PATHS.uplugin, 'utf8'));
}
...
function main() {
	...
	// Parse configs
	const requestedModules = parseSubModulesConfig(PATHS.submodulesConfig);
	...
	if (invalid.length > 0) {
		console.warn(`\n[WARNING] Invalid module names in SubModulesConfig.ini:`);
	...
	const existingUplugin = loadExistingUplugin();
	const newUplugin = generateUplugin(existingUplugin, allModules, requiredPlugins);

	// Write output
	const output = JSON.stringify(newUplugin, null, '  ');
	fs.writeFileSync(PATHS.uplugin, output, 'utf8');
