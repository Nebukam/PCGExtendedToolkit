...
void APCGExValencyAssetPalette::ScanAndRegisterContainedAssets()
{
	...
	// Lambda to add scanned entry
	auto AddScannedEntry = [this, &DetectAssetType](const TSoftObjectPtr<UObject>& Asset, AActor* SourceActor)
	{
		if (Asset.IsNull())
		{
			return;
	...
	for (AActor* Child : ChildActors)
	{
		if (Child && !Child->IsA<APCGExValencyAssetPalette>())
		{
			if (UStaticMeshComponent* SMC = Child->FindComponentByClass<UStaticMeshComponent>())
			{
				if (UStaticMesh* Mesh = SMC->GetStaticMesh())
				{
					AddScannedEntry(TSoftObjectPtr<UObject>(Mesh), Child);
				}
			}
		}
	}
