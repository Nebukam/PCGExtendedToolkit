...
namespace PCGExNoise3D
{
	...
	//
	// In-place single-point generation
	//

	void FNoiseGenerator::GenerateInPlace(const FVector& Position, double& InOutResult, const double InitialWeight) const
	{
		if (Operations.IsEmpty()) { return; }

		const double StartWeight = FMath::Max(SMALL_NUMBER, InitialWeight);

		switch (InPlaceMode)
		{
		case EPCGExNoiseInPlaceMode::Composite:
			{
				double AccumWeight = StartWeight;
				for (int32 i = 0; i < Operations.Num(); ++i)
				{
					const double Value = OperationsPtr[i]->GetDouble(Position);
					const double NewWeight = AccumWeight + Weights[i];
					const double BlendFactor = Weights[i] / NewWeight;
					InOutResult = BlendSingle(BlendModes[i], InOutResult, Value, BlendFactor);
					AccumWeight = NewWeight;
				}
			}
			break;

		case EPCGExNoiseInPlaceMode::BlendResult:
			{
				const double NoiseResult = GetDouble(Position);
				const double BlendFactor = TotalWeight / (StartWeight + TotalWeight);
				InOutResult = BlendSingle(InPlaceBlendMode, InOutResult, NoiseResult, BlendFactor);
			}
			break;

		case EPCGExNoiseInPlaceMode::Additive:
			{
				InOutResult = FMath::Clamp(InOutResult + GetDouble(Position), -1.0, 1.0);
			}
			break;

		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				const double NoiseResult = GetDouble(Position) * 0.5 + 0.5;
				InOutResult *= NoiseResult;
			}
			break;
		}
	}

	void FNoiseGenerator::GenerateInPlace(const FVector& Position, FVector2D& InOutResult, const double InitialWeight) const
	{
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::BlendResult:
			{
				const FVector2D NoiseResult = GetVector2D(Position);
				const double BlendFactor = TotalWeight / (StartWeight + TotalWeight);
				InOutResult = BlendSingle(InPlaceBlendMode, InOutResult, NoiseResult, BlendFactor);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				const FVector2D NoiseResult = GetVector2D(Position);
				InOutResult.X = FMath::Clamp(InOutResult.X + NoiseResult.X, -1.0, 1.0);
				InOutResult.Y = FMath::Clamp(InOutResult.Y + NoiseResult.Y, -1.0, 1.0);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				const FVector2D NoiseResult = GetVector2D(Position);
				InOutResult.X *= NoiseResult.X * 0.5 + 0.5;
				InOutResult.Y *= NoiseResult.Y * 0.5 + 0.5;
			}
			break;
		}
	}

	void FNoiseGenerator::GenerateInPlace(const FVector& Position, FVector& InOutResult, const double InitialWeight) const
	{
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::BlendResult:
			{
				const FVector NoiseResult = GetVector(Position);
				const double BlendFactor = TotalWeight / (StartWeight + TotalWeight);
				InOutResult = BlendSingle(InPlaceBlendMode, InOutResult, NoiseResult, BlendFactor);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				...
				InOutResult.X = FMath::Clamp(InOutResult.X + NoiseResult.X, -1.0, 1.0);
				InOutResult.Y = FMath::Clamp(InOutResult.Y + NoiseResult.Y, -1.0, 1.0);
				InOutResult.Z = FMath::Clamp(InOutResult.Z + NoiseResult.Z, -1.0, 1.0);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				...
				InOutResult.X *= NoiseResult.X * 0.5 + 0.5;
				InOutResult.Y *= NoiseResult.Y * 0.5 + 0.5;
				InOutResult.Z *= NoiseResult.Z * 0.5 + 0.5;
			}
			break;
		}
	}

	void FNoiseGenerator::GenerateInPlace(const FVector& Position, FVector4& InOutResult, const double InitialWeight) const
	{
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::BlendResult:
			{
				const FVector4 NoiseResult = GetVector4(Position);
				const double BlendFactor = TotalWeight / (StartWeight + TotalWeight);
				InOutResult = BlendSingle(InPlaceBlendMode, InOutResult, NoiseResult, BlendFactor);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				...
				InOutResult.Y = FMath::Clamp(InOutResult.Y + NoiseResult.Y, -1.0, 1.0);
				InOutResult.Z = FMath::Clamp(InOutResult.Z + NoiseResult.Z, -1.0, 1.0);
				InOutResult.W = FMath::Clamp(InOutResult.W + NoiseResult.W, -1.0, 1.0);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				...
				InOutResult.Y *= NoiseResult.Y * 0.5 + 0.5;
				InOutResult.Z *= NoiseResult.Z * 0.5 + 0.5;
				InOutResult.W *= NoiseResult.W * 0.5 + 0.5;
			}
			break;
		}
	}
	...
	//
	// In-place batch generation
	//

	void FNoiseGenerator::GenerateInPlace(const TArrayView<const FVector> Positions, TArrayView<double> InOutResults, const double InitialWeight) const
	{
		check(Positions.Num() == InOutResults.Num());
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::Composite:
			{
				...
				double AccumWeight = StartWeight;
				for (int32 OpIdx = 0; OpIdx < Operations.Num(); ++OpIdx)
				{
					OperationsPtr[OpIdx]->Generate(Positions, TempBuffer);
					const double NewWeight = AccumWeight + Weights[OpIdx];
					const double BlendFactor = Weights[OpIdx] / NewWeight;
					BlendBatch(BlendModes[OpIdx], InOutResults.GetData(), TempBuffer.GetData(), Count, BlendFactor);
					AccumWeight = NewWeight;
				}
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::BlendResult:
			{
				TArray<double> NoiseResults;
				NoiseResults.SetNumUninitialized(Count);
				Generate(Positions, NoiseResults);
				const double BlendFactor = TotalWeight / (StartWeight + TotalWeight);
				BlendBatch(InPlaceBlendMode, InOutResults.GetData(), NoiseResults.GetData(), Count, BlendFactor);
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					InOutResults[i] = FMath::Clamp(InOutResults[i] + NoiseResults[i], -1.0, 1.0);
				}
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					InOutResults[i] *= NoiseResults[i] * 0.5 + 0.5;
				}
			}
			break;
		}
	...
	void FNoiseGenerator::GenerateInPlace(const TArrayView<const FVector> Positions, TArrayView<FVector2D> InOutResults, const double InitialWeight) const
	{
		check(Positions.Num() == InOutResults.Num());
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					InOutResults[i].X = FMath::Clamp(InOutResults[i].X + NoiseResults[i].X, -1.0, 1.0);
					InOutResults[i].Y = FMath::Clamp(InOutResults[i].Y + NoiseResults[i].Y, -1.0, 1.0);
				}
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					InOutResults[i].X *= NoiseResults[i].X * 0.5 + 0.5;
					InOutResults[i].Y *= NoiseResults[i].Y * 0.5 + 0.5;
				}
			}
			break;
		}
	...
	void FNoiseGenerator::GenerateInPlace(const TArrayView<const FVector> Positions, TArrayView<FVector> InOutResults, const double InitialWeight) const
	{
		check(Positions.Num() == InOutResults.Num());
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					InOutResults[i].X = FMath::Clamp(InOutResults[i].X + NoiseResults[i].X, -1.0, 1.0);
					InOutResults[i].Y = FMath::Clamp(InOutResults[i].Y + NoiseResults[i].Y, -1.0, 1.0);
					InOutResults[i].Z = FMath::Clamp(InOutResults[i].Z + NoiseResults[i].Z, -1.0, 1.0);
				}
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					InOutResults[i].X *= NoiseResults[i].X * 0.5 + 0.5;
					InOutResults[i].Y *= NoiseResults[i].Y * 0.5 + 0.5;
					InOutResults[i].Z *= NoiseResults[i].Z * 0.5 + 0.5;
				}
			}
			break;
		}
	...
	void FNoiseGenerator::GenerateInPlace(const TArrayView<const FVector> Positions, TArrayView<FVector4> InOutResults, const double InitialWeight) const
	{
		check(Positions.Num() == InOutResults.Num());
		if (Operations.IsEmpty()) { return; }
		...
		case EPCGExNoiseInPlaceMode::Additive:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					...
					InOutResults[i].Y = FMath::Clamp(InOutResults[i].Y + NoiseResults[i].Y, -1.0, 1.0);
					InOutResults[i].Z = FMath::Clamp(InOutResults[i].Z + NoiseResults[i].Z, -1.0, 1.0);
					InOutResults[i].W = FMath::Clamp(InOutResults[i].W + NoiseResults[i].W, -1.0, 1.0);
				}
			}
			break;
		...
		case EPCGExNoiseInPlaceMode::Multiplicative:
			{
				...
				for (int32 i = 0; i < Count; ++i)
				{
					...
					InOutResults[i].Y *= NoiseResults[i].Y * 0.5 + 0.5;
					InOutResults[i].Z *= NoiseResults[i].Z * 0.5 + 0.5;
					InOutResults[i].W *= NoiseResults[i].W * 0.5 + 0.5;
				}
			}
			break;
		}
	...
	//
	// In-place parallel batch generation
	//

	void FNoiseGenerator::GenerateInPlaceParallel(const TArrayView<const FVector> Positions, TArrayView<double> InOutResults, const double InitialWeight, const int32 MinBatchSize) const
	{
		check(Positions.Num() == InOutResults.Num());
		...
		if (Count < MinBatchSize * 2 || Operations.IsEmpty())
		{
			GenerateInPlace(Positions, InOutResults, InitialWeight);
			return;
		}

		ParallelFor(Count, [&](const int32 Index)
		{
			GenerateInPlace(Positions[Index], InOutResults[Index], InitialWeight);
		}, Count < MinBatchSize);
	}

	void FNoiseGenerator::GenerateInPlaceParallel(const TArrayView<const FVector> Positions, TArrayView<FVector2D> InOutResults, const double InitialWeight, const int32 MinBatchSize) const
	{
		check(Positions.Num() == InOutResults.Num());
	...
	void FNoiseGenerator::GenerateInPlaceParallel(const TArrayView<const FVector> Positions, TArrayView<FVector> InOutResults, const double InitialWeight, const int32 MinBatchSize) const
	{
		check(Positions.Num() == InOutResults.Num());
	...
	void FNoiseGenerator::GenerateInPlaceParallel(const TArrayView<const FVector> Positions, TArrayView<FVector4> InOutResults, const double InitialWeight, const int32 MinBatchSize) const
	{
		check(Positions.Num() == InOutResults.Num());
