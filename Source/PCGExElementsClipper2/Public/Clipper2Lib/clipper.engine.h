...
namespace PCGExClipper2Lib
{
	...
	{
		return static_cast<enum VertexFlags>(static_cast<uint32_t>(a) & static_cast<uint32_t>(b));
	}
	...
	struct OutRec;
	using OutRecList = std::vector<OutRec*>;
	...
	using ZCallback64 = std::function<void(const Point64& e1bot, const Point64& e1top,
	                                       const Point64& e2bot, const Point64& e2top, Point64& pt)>;

	using ZCallbackD = std::function<void(const PointD& e1bot, const PointD& e1top,
	                                      const PointD& e2bot, const PointD& e2top, PointD& pt)>;

	using HorzSegmentList = std::vector<HorzSegment>;
	using LocalMinima_ptr = std::unique_ptr<LocalMinima>;
	using LocalMinimaList = std::vector<LocalMinima_ptr>;
	...
	using PolyPath64List = std::vector<std::unique_ptr<PolyPath64>>;
	using PolyPathDList = std::vector<std::unique_ptr<PolyPathD>>;
	...
	public:
		...
		explicit PolyPathD(PolyPathD* parent, const Path64& path)
			: PolyPath(parent)
		{
			scale_ = parent ? parent->scale_ : 1.0;
			int error_code = 0;
		...
		explicit PolyPathD(PolyPathD* parent, const PathD& path)
			: PolyPath(parent)
		{
			scale_ = parent ? parent->scale_ : 1.0;
			polygon_ = path;
