...
namespace PCGExClipper2Lib
{
	...
	constexpr int scale_error_i = 2;      // non-fatal
	constexpr int non_pair_error_i = 4;   // non-fatal
	constexpr int undefined_error_i = 32; // fatal
	constexpr int range_error_i = 64;

#ifdef CLIPPER2_MAX_DECIMAL_PRECISION
	const int CLIPPER2_MAX_DEC_PRECISION = CLIPPER2_MAX_DECIMAL_PRECISION;
#else
	constexpr int CLIPPER2_MAX_DEC_PRECISION = 8; // see Discussions #564
#endif

	static constexpr int64_t MAX_COORD = INT64_MAX >> 2;
	static constexpr int64_t MIN_COORD = -MAX_COORD;
	static constexpr int64_t INVALID = INT64_MAX;
	constexpr double max_coord = static_cast<double>(MAX_COORD);
	constexpr double min_coord = static_cast<double>(MIN_COORD);

	static constexpr double MAX_DBL = (std::numeric_limits<double>::max)();
	...
	struct Point
	{
		...
		void Init(const T2 x_ = 0, const T2 y_ = 0, const z_type z_ = 0)
		{
	...
	Rect<T> GetBounds(const Path<T2>& path)
	{
		...
		for (const auto& p : path)
		{
			if (p.x < xmin)
			{
				xmin = static_cast<T>(p.x);
			}
			if (p.x > xmax)
			{
				xmax = static_cast<T>(p.x);
			}
			if (p.y < ymin)
			{
				ymin = static_cast<T>(p.y);
			}
			if (p.y > ymax)
			{
				ymax = static_cast<T>(p.y);
			}
		}
		return Rect<T>(xmin, ymin, xmax, ymax);
	}
	...
	template <typename T1, typename T2>
	Path<T1> ScalePath(const Path<T2>& path,
	...
	template <typename T1, typename T2>
	Paths<T1> ScalePaths(const Paths<T2>& paths,
	                     double scale_x, double scale_y, int& error_code)
	{
		Paths<T1> result;
	...
	template <typename T1, typename T2>
	Path<T1> TransformPath(const Path<T2>& path)
	{
		Path<T1> result;
		result.reserve(path.size());
	...
	template <typename T1, typename T2>
	Paths<T1> TransformPaths(const Paths<T2>& paths)
	{
		Paths<T1> result;
	...
	template <typename T>
	bool NearEqual(const Point<T>& p1,
	...
	template <typename T>
	Paths<T> StripNearEqual(const Paths<T>& paths,
	                        double max_dist_sqrd, bool is_closed_path)
	{
		Paths<T> result;
		result.reserve(paths.size());
	...
	template <typename T>
	void StripDuplicates(Path<T>& path, bool is_closed_path)
	{
	...
	template <typename T>
	void StripDuplicates(Paths<T>& paths, bool is_closed_path)
	{
	...
	template <typename T>
	int CrossProductSign(const Point<T>& pt1, const Point<T>& pt2, const Point<T>& pt3)
	{
	...
	template <typename T>
	bool IsCollinear(const Point<T>& pt1,
	...
	template <typename T>
	double CrossProduct(const Point<T>& pt1, const Point<T>& pt2, const Point<T>& pt3)
	{
	...
	template <typename T>
	double CrossProduct(const Point<T>& vec1, const Point<T>& vec2)
	{
		return static_cast<double>(vec1.y * vec2.x) - static_cast<double>(vec2.y * vec1.x);
	}

	template <typename T>
	double DotProduct(const Point<T>& pt1, const Point<T>& pt2, const Point<T>& pt3)
	{
	...
	template <typename T>
	double DotProduct(const Point<T>& vec1, const Point<T>& vec2)
	{
	...
	template <typename T>
	double PerpendicDistFromLineSqrd(const Point<T>& pt,
	...
	template <typename T>
	double Area(const Paths<T>& paths)
	{
		double a = 0.0;
	...
	template <typename T>
	bool IsPositive(const Path<T>& poly)
	{
...
#if PCGEX_CLIPPER2_HI_PRECISION
...
#define CC_MAX(x,y) ((x)<(y)?(y):(x))
...
#else
	template <typename T>
	bool GetLineIntersectPt(const Point<T>& ln1a, const Point<T>& ln1b,
	                        const Point<T>& ln2a, const Point<T>& ln2b, Point<T>& ip)
	{
	...
	template <typename T>
	Point<T> TranslatePoint(const Point<T>& pt, double dx, double dy)
	{
	...
	template <typename T>
	Point<T> ReflectPoint(const Point<T>& pt, const Point<T>& pivot)
	{
		return Point<T>(pivot.x + (pivot.x - pt.x), pivot.y + (pivot.y - pt.y), pt.z);
	}

	template <typename T>
	int GetSign(const T& val)
	{
		if (!val)
		{
			return 0;
	...
	template <typename T>
	Point<T> GetClosestPointOnSegment(const Point<T>& offPt,
