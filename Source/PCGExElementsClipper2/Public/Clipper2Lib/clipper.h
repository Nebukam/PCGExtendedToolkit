...
namespace PCGExClipper2Lib
{
	...
	template <typename T>
	Path<T> TranslatePath(const Path<T>& path, T dx, T dy)
	{
		Path<T> result;
	...
	template <typename T>
	Paths<T> TranslatePaths(const Paths<T>& paths, T dx, T dy)
	{
		Paths<T> result;
	...
	namespace details
	{
		                              ...
		                              size_t idx, std::string preamble)
		{
			...
			for (size_t i = 0; i < pp.Count(); ++i)
			{
				if (pp.Child(i)->Count())
				{
					OutlinePolyPath64(os, *pp.Child(i), i, preamble + "  ");
				}
			}
		}
		                             ...
		                             size_t idx, std::string preamble)
		{
			...
			for (size_t i = 0; i < pp.Count(); ++i)
			{
				if (pp.Child(i)->Count())
				{
					OutlinePolyPathD(os, *pp.Child(i), i, preamble + "  ");
				}
			}
		}

		template <typename T, typename U>
		constexpr void MakePathGeneric(const T an_array,
	...
	template <typename T,
	          std::enable_if_t<
		          std::is_integral_v<T> &&
		          !std::is_same_v<char, T>, bool> = true>
	Path64 MakePath(const std::vector<T>& list)
	{
	...
	template <typename T, std::size_t N,
	          std::enable_if_t<
		          std::is_integral_v<T> &&
		          !std::is_same_v<char, T>, bool> = true>
	Path64 MakePath(const T (&list)[N])
	{
	...
	template <typename T,
	          std::enable_if_t<
		          ...
		          !std::is_same_v<char, T>, bool> = true>
	PathD MakePathD(const std::vector<T>& list)
	{
		const auto size = list.size() - list.size() % 2;
		if (list.size() != size)
		{
	...
	template <typename T, std::size_t N,
	          std::enable_if_t<
		          std::is_arithmetic_v<T> &&
		          !std::is_same_v<char, T>, bool> = true>
	PathD MakePathD(const T (&list)[N])
	{
		// Make the compiler error on unpaired value (i.e. no runtime effects).
		static_assert(N % 2 == 0, "MakePath requires an even number of arguments");
		PathD result;
	...
	Path64 MakePathZ(const T2 (&list)[N])
	{
	...
	template <typename T2, std::size_t N>
	PathD MakePathZD(const T2 (&list)[N])
	{
		static_assert(N % 3 == 0,
	...
	template <typename T>
	double Length(const Path<T>& path, bool is_closed_path = false)
	{
		double result = 0.0;
		if (path.size() < 2)
		{
	...
	template <typename T>
	Path<T> Ellipse(const Rect<T>& rect, size_t steps = 0)
	{
	...
	template <typename T>
	Path<T> Ellipse(const Point<T>& center,
	                double radiusX, double radiusY = 0, size_t steps = 0)
	{
		...
		if (steps <= 2)
		{
			steps = static_cast<size_t>(UE_PI * sqrt((radiusX + radiusY) / 2));
		}

		double si = std::sin(2 * UE_PI / steps);
		double co = std::cos(2 * UE_PI / steps);
		double dx = co, dy = si;
		Path<T> result;
	...
	template <typename T>
	Path<T> SimplifyPath(const Path<T>& path,
	...
	template <typename T>
	Paths<T> SimplifyPaths(const Paths<T>& paths,
	                       double epsilon, bool isClosedPath = true)
	{
		Paths<T> result;
		result.reserve(paths.size());
	...
	template <typename T>
	bool Path2ContainsPath1(const Path<T>& path1, const Path<T>& path2)
	{
	...
	template <typename T>
	void RDP(const Path<T> path, std::size_t begin,
	...
	template <typename T>
	Path<T> RamerDouglasPeucker(const Path<T>& path, double epsilon)
	{
	...
	template <typename T>
	Paths<T> RamerDouglasPeucker(const Paths<T>& paths, double epsilon)
	{
		Paths<T> result;
		result.reserve(paths.size());
		std::transform(paths.begin(), paths.end(), back_inserter(result),
