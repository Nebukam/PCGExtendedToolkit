...
namespace PCGExClipper2Lib
{
	...
	constexpr double arc_const = 0.002; // <-- 1/500
	...
	void ClipperOffset::DoRound(const Path64& path, size_t j, size_t k, double angle)
	{
		if (deltaCallback64_)
		{
			...
			double arcTol = (arc_tolerance_ > floating_point_tolerance ? std::min(abs_delta, arc_tolerance_) : abs_delta * arc_const);
			double steps_per_360 = std::min(UE_PI / std::acos(1 - arcTol / abs_delta), abs_delta * UE_PI);
			step_sin_ = std::sin(2 * UE_PI / steps_per_360);
			step_cos_ = std::cos(2 * UE_PI / steps_per_360);
			if (group_delta_ < 0.0)
			{
				step_sin_ = -step_sin_;
			}
			steps_per_rad_ = steps_per_360 / (2 * UE_PI);
		}
	...
	void ClipperOffset::OffsetOpenPath(Group& group, const Path64& path)
	{
		...
		else
		{
			...
			case EndType::Butt:
				...
				break;
			case EndType::Round:
				DoRound(path, highI, highI, UE_PI);
				break;
			default:
	...
	void ClipperOffset::DoGroupOffset(Group& group)
	{
		...
		//double min_area = UE_PI * Sqr(group_delta_);
		...
		for (; path_in_it != group.paths_in.cend(); ++path_in_it)
		{
			...
			if (pathLen == 1) // single point
			{
				...
				if (group.join_type == JoinType::Round)
				{
					...
					size_t steps = steps_per_rad_ > 0 ? static_cast<size_t>(std::ceil(steps_per_rad_ * 2 * UE_PI)) : 0; //#617
				...
				{
					int d = static_cast<int>(std::ceil(abs_delta));
