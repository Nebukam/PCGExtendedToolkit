...
namespace PCGExDataBlending
{
	...
	public:
		...
		FORCEINLINE void Blend(const int32 SourceAIndex, const int32 SourceBIndex, const int32 TargetIndex, const double Weight = 1) const
		{
			for (int i = 0; i < Operations->Num(); i++) { (*(Operations->GetData() + i))->Blend(SourceAIndex, SourceBIndex, TargetIndex); }
		}

		FORCEINLINE void Blend(const int32 SourceIndex, const int32 TargetIndex, const double Weight = 1) const
		{
