...
namespace PCGExTypeOps
{
	...
	struct FTypeOps<FVector2D>
	{
		...
		static constexpr EPCGMetadataTypes TypeId = EPCGMetadataTypes::Vector2;
		...
		static Type ConvertFrom(const TFrom& Value)
		{
			...
			else if constexpr (std::is_same_v<TFrom, FQuat>) { const FRotator R = Value.Rotator(); return Type(R.Pitch, R.Yaw); }
			else if constexpr (std::is_same_v<TFrom, FRotator>) { return Type(Value.Pitch, Value.Yaw); }
			else if constexpr (std::is_same_v<TFrom, FTransform>) { const FVector L = Value.GetLocation(); return Type(L.X, L.Y); }
			else if constexpr (std::is_same_v<TFrom, FString>) { Type Result; Result.InitFromString(Value); return Result; }
			else if constexpr (std::is_same_v<TFrom, FName>) { Type Result; Result.InitFromString(Value.ToString()); return Result; }
	...
	{
		using Type = FVector;
		static constexpr EPCGMetadataTypes TypeId = EPCGMetadataTypes::Vector;
		...
		static Type ConvertFrom(const TFrom& Value)
		{
			...
			else if constexpr (std::is_same_v<TFrom, FQuat>) { const FRotator R = Value.Rotator(); return Type(R.Pitch, R.Yaw, R.Roll); }
	...
	{
		using Type = FVector4;
		static constexpr EPCGMetadataTypes TypeId = EPCGMetadataTypes::Vector4;
		...
		static Type ConvertFrom(const TFrom& Value)
		{
			if constexpr (std::is_same_v<TFrom, bool>) { const double D = Value ? 1.0 : 0.0; return Type(D, D, D, D); }
			...
			else if constexpr (std::is_same_v<TFrom, FTransform>) { const FVector L = Value.GetLocation(); return Type(L.X, L.Y, L.Z, 0.0); }
			else if constexpr (std::is_same_v<TFrom, FString>) { Type Result; Result.InitFromString(Value); return Result; }
			else if constexpr (std::is_same_v<TFrom, FName>) { Type Result; Result.InitFromString(Value.ToString()); return Result; }
