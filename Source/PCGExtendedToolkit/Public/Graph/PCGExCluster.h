...
{
#pragma region Cluster Parallel Batch toolkit
	...
	public:
		...
		virtual void CompleteWork()
		{
			for (FClusterProcessingData* Data : Processors) { Data->CompleteWork(); }
		}
	};
	...
	{
		Manager->Start<PCGExClusterTask::FStartClusterBatchProcessing<T>>(-1, nullptr, Batch);
	}

	template <typename T>
	static void CompleteBatches(FPCGExAsyncManager* Manager, TArrayView<T*> Batches)
	{
		for (T* Batch : Batches) { Manager->Start<PCGExClusterTask::FStartClusterBatchCompleteWork<T>>(-1, nullptr, Batch); }
	}
