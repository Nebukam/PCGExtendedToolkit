...
#include "Graph/Filters/Edges/PCGExEdgeDirectionFilter.h"
...
bool FIsoEdgeDirectionFilter::Init(const FPCGContext* InContext, const TSharedPtr<PCGExCluster::FCluster>& InCluster, const TSharedPtr<PCGExData::FFacade>& InPointDataFacade, const TSharedPtr<PCGExData::FFacade>& InEdgeDataFacade)
{
...
bool FIsoEdgeDirectionFilter::Test(const PCGExCluster::FNode& Node) const
{
	return bUseDot ? TestDot(Node) : TestHash(Node);
}

bool FIsoEdgeDirectionFilter::TestDot(const PCGExCluster::FNode& Node) const
{
...
bool FIsoEdgeDirectionFilter::TestHash(const PCGExCluster::FNode& Node) const
{
	const int32 PointIndex = Node.PointIndex;
	const TArray<PCGExCluster::FNode>& NodesRef = *Cluster->Nodes;
