...
namespace PCGExFloodFill
{
	...
	void FDiffusion::Diffuse(
		...
		const TArray<TSharedPtr<PCGExAttributeBlendOperation>>& Operations,
		TArray<int32>& OutIndices)
	{
		...
		for (int i = 0; i < OutIndices.Num(); i++)
		{
			...
			if (TargetIndex != SourceIndex)
			{
				...
				for (const TSharedPtr<PCGExAttributeBlendOperation>& Op : Operations) { Op->Blend(SourceIndex, SourcePoint, TargetIndex, TargetPoint); }
			}
		}
	}
	...
	bool FFillControlsHandler::BuildFrom(FPCGExContext* InContext, const TArray<TObjectPtr<const UPCGExFillControlsFactoryData>>& InFactories)
	{
		if (!InFactories.IsEmpty())
		{
			...
			{
				TSharedPtr<PCGExFillControlOperation> Op = Factory->CreateOperation(InContext);
				if (!Op) { return false; }
	...
	bool FFillControlsHandler::PrepareForDiffusions(const TArray<TSharedPtr<FDiffusion>>& Diffusions, const FPCGExFloodFillFlowDetails& Details)
	{
		...
		for (const TSharedPtr<PCGExFillControlOperation>& Op : Operations)
		{
	...
	{
		for (const TSharedPtr<PCGExFillControlOperation>& Op : SubOpsCapture) { if (!Op->IsValidCapture(Diffusion, Candidate)) { return false; } }
	...
	{
		for (const TSharedPtr<PCGExFillControlOperation>& Op : SubOpsProbe) { if (!Op->IsValidProbe(Diffusion, Candidate)) { return false; } }
		return true;
	}

	bool FFillControlsHandler::IsValidCandidate(const FDiffusion* Diffusion, const FCandidate& From, const FCandidate& Candidate)
	{
		for (const TSharedPtr<PCGExFillControlOperation>& Op : SubOpsCandidate) { if (!Op->IsValidCandidate(Diffusion, From, Candidate)) { return false; } }
		return true;
	}
}
