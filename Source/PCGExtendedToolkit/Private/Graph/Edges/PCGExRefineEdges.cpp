...
TArray<FPCGPinProperties> UPCGExRefineEdgesSettings::InputPinProperties() const
{
	...
	if (Refinement && Refinement->WantsHeuristics()) { PCGEX_PIN_FACTORIES(PCGExGraph::SourceHeuristicsLabel, "Heuristics may be required by some refinements.", Required, FPCGExHeuristicDataTypeInfo) }
	if (Refinement && Refinement->SupportFilters())
	{
		...
		PCGEX_PIN_FACTORIES(PCGExGraph::SourceEdgeFiltersLabel, "Refinements filters.", Normal, FPCGExEdgeFilterDataTypeInfo)
	}

	if (Sanitization == EPCGExRefineSanitization::Filters)
	{
		PCGEX_PIN_FACTORIES(PCGExRefineEdges::SourceSanitizeEdgeFilters, "Filters that define which edges are to be kept. During the sanitization step, edges that pass the filters are restored if they were previously removed.", Required, FPCGExEdgeFilterDataTypeInfo)
	}
