...
bool FPCGExUnpackClustersElement::Boot(FPCGContext* InContext) const
{
	...
	Context->OutPoints = new PCGExData::FPointIOGroup();
	Context->OutPoints->DefaultOutputLabel = PCGExGraph::OutputVerticesLabel;

	Context->OutEdges = new PCGExData::FPointIOGroup();
	...
	FPCGContext* InContext) const
{
	...
	if (Context->IsState(PCGExMT::State_ReadyForNextPoints))
	{
		while (Context->AdvancePointsIO())
		{
			...
			//TODO: Read info from attributes
			...
			TArray<FPCGPoint>& MutablePoints = NewEdges.GetOut()->GetMutablePoints();
			MutablePoints.RemoveAt(NumEdges, NumVtx);
			...
			MutablePoints = NewVtx.GetOut()->GetMutablePoints();
			MutablePoints.RemoveAt(0, NumEdges);
