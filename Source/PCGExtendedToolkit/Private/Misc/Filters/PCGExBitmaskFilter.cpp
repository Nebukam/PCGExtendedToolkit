...
bool PCGExPointsFilter::TBitmaskFilter::Init(const FPCGContext* InContext, PCGExData::FFacade* InPointDataFacade)
{
	...
	FlagsReader = PointDataFacade->GetOrCreateFetchReader<int64>(TypedFilterFactory->Config.FlagsAttribute);
	...
	{
		MaskReader = PointDataFacade->GetOrCreateFetchReader<int64>(TypedFilterFactory->Config.BitmaskAttribute);
		if (!MaskReader)
		{
