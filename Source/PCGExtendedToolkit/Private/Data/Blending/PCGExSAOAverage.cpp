...
bool UPCGExSAOAverage##_NAME::UseFinalize() { return true; }\
void UPCGExSAOAverage##_NAME::DoOperation(const PCGMetadataEntryKey OperandAKey, const PCGMetadataEntryKey OperandBKey, const PCGMetadataEntryKey OutputKey, const double Alpha){ PCGEX_DECL(_TYPE) PCGEX_SET(_SET) };\
void UPCGExSAOAverage##_NAME::FinalizeOperation(PCGMetadataEntryKey OutputKey, double Alpha){ const _TYPE Val = GetValue(OutputKey); PCGEX_SET(_SET_FINAL) };
