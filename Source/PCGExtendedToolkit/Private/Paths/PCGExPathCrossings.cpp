...
TArray<FPCGPinProperties> UPCGExPathCrossingsSettings::InputPinProperties() const
{
	...
	PCGEX_PIN_FACTORIES(PCGExPaths::SourceCanCutFilters, "Fiter which edges can 'cut' other edges. Leave empty so all edges are can cut other edges.", Normal, FPCGExDataTypeInfoFilterPoint::AsId())
	PCGEX_PIN_FACTORIES(PCGExPaths::SourceCanBeCutFilters, "Fiter which edges can be 'cut' by other edges. Leave empty so all edges are can cut other edges.", Normal, FPCGExDataTypeInfoFilterPoint::AsId())
