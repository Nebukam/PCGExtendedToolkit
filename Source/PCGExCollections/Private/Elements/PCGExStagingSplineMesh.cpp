...
void FPCGExPathSplineMeshContext::RegisterAssetDependencies()
{
	...
	if (!Settings->bUseStagedPoints && Settings->CollectionSource == EPCGExCollectionSource::Asset)
	{
	...
	PCGEX_ON_ASYNC_STATE_READY(PCGExCommon::States::State_WaitingOnAsyncWork)
	{
		...
		PCGExHelpers::Load(
			...
			[&, Ctx = Context->GetOrCreateHandle()](const bool bSuccess, TSharedPtr<FStreamableHandle> StreamableHandle)
			{
				...
				else { SharedContext.Get()->CancelExecution(); }
			}
		);
	}
	...
	void FProcessor::ProcessPoints(const PCGExMT::FScope& Scope)
	{
		...
		PCGEX_SCOPE_LOOP(Index)
		{
			...
			if (!PointFilterCache[Index]
				|| (Index == LastIndex && !bClosedLoop)
				|| (bLocalFitting && !Source->TryGetHelpers(Index, Helper, MicroHelper)))
			{
				InvalidPoint(Index);
				continue;
			}
