...
namespace PCGExBitmask
{
	...
	PCGEXFOUNDATIONS_API FString ToString(const EPCGExBitflagComparison Comparison);
	...
	PCGEXFOUNDATIONS_API bool Compare(const EPCGExBitflagComparison Method, const int64& Flags, const int64& Mask);
	...
	PCGEXFOUNDATIONS_API void Mutate(const TArray<FPCGExBitmaskRef>& Compositions, int64& Flags);

	PCGEXFOUNDATIONS_API void Mutate(const TArray<FPCGExSimpleBitmask>& Compositions, int64& Flags);

	struct PCGEXFOUNDATIONS_API FCachedRef
	{
		FCachedRef() = default;
