...
namespace PCGExCluster
{
	...
	bool FCluster::BuildFrom(const TMap<uint32, int32>& InEndpointsLookup, const TArray<int32>* InExpectedAdjacency)
	{
		...
		const TUniquePtr<PCGExData::TArrayBuffer<int64>> EndpointsBuffer = MakeUnique<PCGExData::TArrayBuffer<int64>>(PinnedEdgesIO.ToSharedRef(), PCGExGraph::Labels::Attr_PCGExEdgeIdx);
		...
		else if (BoundedEdges)
		{
			for (const FBoundedEdge& Edge : (*BoundedEdges))
			{
				...
				if (Dist < MaxDistance)
				{
					if (MinNeighbors > 0)
					{
						if (GetEdgeStart(Edge.Index)->Links.Num() < MinNeighbors && GetEdgeEnd(Edge.Index)->Links.Num() < MinNeighbors)
						{
							continue;
						}
					}
