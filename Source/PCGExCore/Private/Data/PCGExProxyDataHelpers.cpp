...
#define PCGEX_CREATE_CONST_PROXY(_TYPE, _NAME, ...) \
			...
			{ \
				...
				{ \
					const FPCGMetadataAttribute<_TYPE>* Attr = PCGExMetaHelpers::TryGetConstAttribute<_TYPE>( \
						InDataFacade->GetIn(), \
						PCGExMetaHelpers::GetAttributeIdentifier(InDescriptor.Selector, InDataFacade->GetIn())); \
					if (Attr) \
					{ \
			...
			if (!OutProxy->Validate(InDescriptor))
			{
				PCGE_LOG_C(Error, GraphAndLog, InContext,
				           FText::Format(
					           ...
					           FText::FromString(PCGExMetaHelpers::GetSelectorDisplayName(InDescriptor.Selector))));
				OutProxy = nullptr;
			}
		}
...
#define PCGEX_TPL(_TYPE, _NAME, ...) \
	template PCGEXCORE_API TSharedPtr<IBufferProxy> GetConstantProxyBuffer<_TYPE>(const _TYPE& Constant);
	PCGEX_FOREACH_SUPPORTEDTYPES(PCGEX_TPL)
#undef PCGEX_TPL
