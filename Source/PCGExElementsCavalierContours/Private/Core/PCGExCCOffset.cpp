		...
		namespace Internal
		{
				...
				double OffsetTolerance)
			{
				...
				auto IntersectsOriginalPolyline = [&](const FVertex& V1, const FVertex& V2) -> bool
				{
					...
					const double Fuzz = Math::FuzzyEpsilon;
					OrigIndex.Query(MinX - Fuzz, MinY - Fuzz, MaxX + Fuzz, MaxY + Fuzz, [&](int32 SegIdx)
					{
						...
						const FPlineSegIntersect Intr = PlineSegmentIntersect(V1, V2, U1, U2, PosEqualEps);
						if (Intr.Type != EPlineSegIntersectType::NoIntersect)
						{
							bHasIntersect = true;
						}
					});
				...
				auto SliceIsValid = [&](int32 StartIdx, int32 TraverseCount, const FVector2D& StartPt, const FVector2D& EndPt) -> bool
				{
					...
					if (TraverseCount == 0)
					{
						...
						if (!PointValidForOffset(Original, OrigIndex, Offset, StartPt, PosEqualEps, OffsetTolerance))
							return false;
						if (!PointValidForOffset(Original, OrigIndex, Offset, EndPt, PosEqualEps, OffsetTolerance))
							return false;
						...
						if (!PointValidForOffset(Original, OrigIndex, Offset, MidPt, PosEqualEps, OffsetTolerance))
							return false;
					...
					if (!PointValidForOffset(Original, OrigIndex, Offset, StartMid, PosEqualEps, OffsetTolerance))
						return false;
					...
					if (!PointValidForOffset(Original, OrigIndex, Offset, EndMid, PosEqualEps, OffsetTolerance))
						return false;
					...
					for (int32 i = 1; i <= TraverseCount; ++i)
					{
						...
						if (!PointValidForOffset(Original, OrigIndex, Offset, PrevV.GetPosition(), PosEqualEps, OffsetTolerance))
							return false;

						if (IntersectsOriginalPolyline(PrevV, CurV))
							return false;
					...
					if (IntersectsOriginalPolyline(PrevV, EndPtV))
						return false;
				...
				auto TryAddSlice = [&](const FVector2D& StartPt, int32 StartIdx, const FVector2D& EndPt, int32 EndIdx)
				{
					...
					if (TraverseCount < 0 && Original.IsClosed())
					{
						TraverseCount += N;
					}
					...
					if (TraverseCount == 0 && Original.IsClosed() && !StartPt.Equals(EndPt, PosEqualEps))
					{
						...
						if (Math::DistanceSquared(SegStart, StartPt) >= Math::DistanceSquared(SegStart, EndPt))
						{
							TraverseCount = N;
						}
					}
					...
					// Full validation
					if (!SliceIsValid(StartIdx, TraverseCount, StartPt, EndPt))
					{
						return;
					}
